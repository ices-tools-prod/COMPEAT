...
path <- "D:/COMPEAT/CPHL_EO_RBINS/v1.01"

# Convert netCDF into CSV if it hasn't already been done
if (length(list.files(file.path(path, "csv"))) == 0) {
  filenames <- list.files(path = file.path(path, "CHL_daily_2015_2020"), pattern = "*.nc", full.names = TRUE)
...
# Write data table
fwrite(dt7, file.path("D:/COMPEAT", "Indicator_CPHL_EO_RBINS_02.csv"))
