...
```{r mann kendall}
#this requires that you run the main compeat script up to the point of generating wk3 first.
wk <- wk3[wk3$IndicatorID == 4,]
trends <- wk[order(wk$UnitID,wk$Period),]
trends <- wk[order(wk$UnitID),]
str(trends)
trends <- subset(trends,!is.na(trends$ES))
polygon <- trends$UnitID
polygon_nonnull <- !is.na(polygon)
polygon <- polygon[polygon_nonnull]
year <- as.integer(paste(trends$Period))
year <- year[polygon_nonnull]
str(year)
year
level <- trends$ES
level <- level[polygon_nonnull]
#install.packages("emon")
library(emon)
poly.names <- as.character(as.data.frame(table(polygon))$polygon)
table(poly.names)
#poly.names <- poly.names[-7]         # Not using "Undefined"
...
for (j in poly.names) {
  ...
  year1 <- year[index]
  level1 <- level[index]
  temp <- mannkendall(year1, level1, nsims.mk=9999)
  mkstat[count] <- temp$mann
  mkp[count] <- temp$pvalue
  plot(year1, level1, main=j)
  tt <- lm(level1 ~ year1)
  lines(year1, tt$fitted.values)
}


mkstat                               # Mann-Kendall statistic
mkp                                  # p-values
...
colnames(mk_results_oxy) <- c("UnitID","No_Years")
mk_results_oxy$mkstat <- mkstat
mk_results_oxy$mkp <- mkp
mk_results_oxy$UnitID <- as.integer(mk_results_oxy$UnitID)

mk_results_oxy <- merge(st_drop_geometry(units[1:4]), mk_results_oxy, by = "UnitID")
